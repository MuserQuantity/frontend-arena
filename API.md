...
GET /model-icons/{vendor}.svg            # 模型图标（按厂商命名：claude/gemini/glm/gpt...）
```
其中 `NN` 为 `index` 的两位零填充（0 → `00`）；图标按厂商命名，同厂商多版本共用。
...
模型图标：把 svg 放进 `client/public/model-icons/`，**按厂商命名**（如 `deepseek.svg`），
同厂商的多个版本共用一个图标，`icon_url` 填 `/model-icons/<厂商>.svg`。
